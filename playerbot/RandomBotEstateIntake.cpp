...
std::future<bool> RandomBotEstateStore::BeginIntake(RandomBotEstateIntake r)
{
    ...
    return CharacterDatabase.QueueTransaction([r](SqlConnection& c)
    {
        ...
        return c.Execute(("UPDATE ai_playerbot_lifecycle SET starting_gold="+std::to_string(gold)+",estate_escrow="+
            std::to_string(gold)+",disposition="+std::to_string(r.disposition)+",replacement_required="+
...
std::future<bool> RandomBotEstateStore::TransferMail(RandomBotEstateMailIntake r)
{
    ...
    return CharacterDatabase.QueueTransaction([r](SqlConnection& c)
    {
        ...
        auto mail=c.Query(("SELECT receiver,money,cod,has_items FROM mail WHERE id="+std::to_string(r.mailId)+" FOR UPDATE").c_str());
        if(!mail)return false;Field* m=mail->Fetch();
        if(m[0].GetUInt32()!=r.originalGuid||m[1].GetUInt64()!=r.money||m[2].GetUInt32()||m[3].GetBool()!=!r.items.empty())return false;
        ...
        return c.Execute(("DELETE FROM mail_items WHERE mail_id="+std::to_string(r.mailId)).c_str())&&
            ...
            c.Execute(("UPDATE ai_playerbot_estate SET escrow=escrow+"+std::to_string(r.money)+",updated_at=UNIX_TIMESTAMP() WHERE estate_id="+
...
std::future<bool> RandomBotEstateStore::AdoptAuction(RandomBotEstateAuctionIntake r)
{
    ...
    return CharacterDatabase.QueueTransaction([r](SqlConnection& c)
    {
        if(!r.estateId||!r.originalGuid||!r.brokerGuid||!r.auctionId||!r.itemGuid||!r.itemEntry||!r.itemCount)return false;
        ...
        if(f[0].GetUInt32()!=r.houseId||f[1].GetUInt32()!=r.itemGuid||f[2].GetUInt32()!=r.itemEntry||f[3].GetUInt32()!=r.itemCount||
            f[4].GetUInt32()!=r.originalGuid||f[5].GetUInt32()!=r.buyout||f[6].GetUInt64()!=r.expiresAt||f[7].GetUInt64()||
...
std::future<bool> RandomBotEstateStore::FinalizeIntake(uint64 estateId,uint32 originalGuid,uint32 originalAccount)
{
    ...
    return CharacterDatabase.QueueTransaction([estateId,originalGuid,originalAccount](SqlConnection& c)
    {
        ...
        auto e=c.Query(("SELECT original_guid,original_account,status,assets_detached FROM ai_playerbot_estate WHERE estate_id="+
            std::to_string(estateId)+" FOR UPDATE").c_str());if(!e||e->Fetch()[0].GetUInt32()!=originalGuid||
            e->Fetch()[1].GetUInt32()!=originalAccount)return false;if(e->Fetch()[3].GetBool())return true;
